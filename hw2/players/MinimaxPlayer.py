...
class Player(AbstractPlayer):
    ...
    def __init__(self, game_time, penalty_score):
        ...
        self.board = None
        self.pos = None
        self.mini_max = SearchAlgos.MiniMax(self.utility, self.succ, self.make_move)
        self.fruit = None
        self.my_score = 0
        self.rival_score = 0
        self.sum_fruit = 0
    ...
    def make_move(self, time_limit, players_score):
        ...
        while total_time + 3*curr_time < time_limit:
            start = time.time()
            min_max_value, next_pos = self.mini_max.search(self, depth, True)
            if best_min_max_value < min_max_value:
                best_min_max_value = min_max_value
                best_next_pos = next_pos
            end = time.time()
            curr_time = end - start  #check if need += or =???
    ...
    def succ(self, pos):
        ...
        for d in self.directions:
            i = pos[0] + d[0]
            j = pos[1] + d[1]
            if 0 <= i < len(self.board) and 0 <= j < len(self.board[0]) and (self.board[int(i)][int(j)] not in [-1, 1, 2]):   # then move is legal
