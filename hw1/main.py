...
import numpy as np
from typing import List, Union, Optional, FrozenSet
...
def plot_distance_and_expanded_wrt_weight_figure(
    ...
    p1, = ax1.plot(weights, total_cost, label="Solution cost",
                   color="blue")  # TODO: pass the relevant params instead of `...`.
    ...
    p2, = ax2.plot(weights, total_nr_expanded, label="#Expanded states",
                   color="red")  # TODO: pass the relevant params instead of `...`.
...
def run_astar_for_weights_in_range(heuristic_type: HeuristicFunctionType, problem: GraphProblem, n: int = 30,
    ...
    weight_array = np.linspace(low_heuristic_weight, high_heuristic_weight, n)
    cost = []
    expanded = []
    weight = []
    for e in weight_array:
        a_star_air_dist = AStar(heuristic_type, e, max_nr_states_to_expand)
        res = a_star_air_dist.solve_problem(problem)
...
def toy_map_problem_experiments():
    ...
    a_star_air_dist = AStar(AirDistHeuristic)
    res = a_star_air_dist.solve_problem(toy_map_problem)
    print(res)
...
def multiple_objectives_mda_problem_experiments():
    ...
    a_star = AStar(MDAMSTAirDistHeuristic)
    res = a_star.solve_problem(moderate_mda_problem_with_distance_cost)
    optimal_distance_cost = res.solution_cost.get_g_cost()
    eps = 0.6
    max_distance_cost = optimal_distance_cost * (1+eps)
    a_star_nearest_lab = AStar(MDATestsTravelDistToNearestLabHeuristic,
                               open_criterion=(lambda x: x.cost.distance_cost <= max_distance_cost))
    res = a_star_nearest_lab.solve_problem(moderate_mda_problem_with_tests_travel_dist_cost)
    print(res)
...
def mda_problem_with_astar_epsilon_experiments():
    ...
    a_star = AStar(MDAMSTAirDistHeuristic)
    res = a_star.solve_problem(small_mda_problem_with_distance_cost)
    print(res)
    ...
    a_star_epsilon = AStarEpsilon(MDAMSTAirDistHeuristic, within_focal_h_sum_priority_function, focal_epsilon=0.23,
                                  max_focal_size=40)
...
def mda_problem_anytime_astar_experiments():
    ...
    any_time_astar = AnytimeAStar(MDAMSTAirDistHeuristic, max_nr_states_to_expand_per_iteration=1000)
    res = any_time_astar.solve_problem(moderate_mda_problem_with_distance_cost)
    print(res)
