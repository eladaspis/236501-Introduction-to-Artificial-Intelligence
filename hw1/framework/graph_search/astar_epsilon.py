...
class AStarEpsilon(AStar):
    ...
    def _extract_next_search_node_to_expand(self, problem: GraphProblem) -> Optional[SearchNode]:
        ...
        """
        f_min_value = self.open.peek_next_node().expanding_priority
        if f_min_value is None:
            return None
        focal = []
        if self.max_focal_size is None:
            while not self.open.is_empty():
                node = self.open.peek_next_node()
                if node.expanding_priority <= (1 + self.focal_epsilon) * f_min_value:
                    focal.append(self.open.pop_next_node())
                else:
                    break
        else:
            while not self.open.is_empty() and len(focal) <= self.max_focal_size:
                node = self.open.peek_next_node()
                if node.expanding_priority <= (1 + self.focal_epsilon) * f_min_value:
                    focal.append(self.open.pop_next_node())
                else:
        ...
        min_search_node = None
        min_priority_search_node = float('inf')
        for node in focal:
            if self.within_focal_priority_function(node, problem, self) < min_priority_search_node:
                min_priority_search_node = self.within_focal_priority_function(node, problem, self)
                min_search_node = node

        for search_node in focal:
            if search_node != min_search_node:
                self.open.push_node(search_node)

        if self.use_close:
            self.close.add_node(min_search_node)

        return min_search_node
