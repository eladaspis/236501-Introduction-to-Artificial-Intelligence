    ...
    def solve_problem(self, problem: GraphProblem) -> SearchResult:
        with Timer(print_title=False) as timer:
            ...
            while (high_heuristic_weight - low_heuristic_weight) > 0.01:
                ...
                a_star_weighted_mid = AStar(heuristic_function_type, heuristic_weight, max_nr_states_to_expand)
                res_mid = a_star_weighted_mid.solve_problem(problem)
                total_nr_expanded_states += res_mid.nr_expanded_states
                max_nr_stored_states = max(max_nr_stored_states, res_mid.max_nr_stored_states)
                if res_mid.is_solution_found:
                    high_heuristic_weight = heuristic_weight
                    # Update best values
                    if res_mid.solution_g_cost < best_solution.solution_g_cost:
                        best_solution = res_mid
