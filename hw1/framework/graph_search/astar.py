...
class AStar(BestFirstSearch):
    ...
    def _open_successor_node(self, problem: GraphProblem, successor_node: SearchNode):
        ...
        if self.close.has_state(successor_node.state):
            if successor_node.g_cost < self.close.get_node_by_state(successor_node.state).g_cost:
                self.close.remove_node(self.close.get_node_by_state(successor_node.state))
            else:
                return
        if self.open.has_state(successor_node.state):
            if successor_node.g_cost < self.open.get_node_by_state(successor_node.state).g_cost:
