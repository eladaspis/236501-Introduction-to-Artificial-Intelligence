...
class MDAMaxAirDistHeuristic(HeuristicFunction):
    ...
    def estimate(self, state: GraphProblemState) -> float:
        ...
        if len(all_certain_junctions_in_remaining_ambulance_path) < 2:
            return 0
        max_nr = max(self.cached_air_distance_calculator.get_air_distance_between_junctions(item1, item2)
...
class MDASumAirDistHeuristic(HeuristicFunction):
    ...
    def estimate(self, state: GraphProblemState) -> float:
        ...
        count = 0
        temp = state.current_location
        available_junctions = frozenset(frozenset(all_certain_junctions_in_remaining_ambulance_path) -
                                        {state.current_location})
        while available_junctions != frozenset():
            min_junction = float('inf')
            for item in available_junctions:
                if self.cached_air_distance_calculator.get_air_distance_between_junctions(temp, item) <= min_junction:
                    if self.cached_air_distance_calculator.\
                            get_air_distance_between_junctions(temp, item) == min_junction:
                        min_junction = item.index if item.index <= curr_junction.index else curr_junction.index
                        curr_junction = item if item.index <= curr_junction.index else curr_junction
                    else:
                        curr_junction = item
                        min_junction = self.cached_air_distance_calculator.\
                            get_air_distance_between_junctions(temp, item)
            available_junctions = frozenset(available_junctions - {curr_junction})
            count += self.cached_air_distance_calculator.get_air_distance_between_junctions(curr_junction, temp)
            temp = curr_junction
        return count
...
class MDAMSTAirDistHeuristic(HeuristicFunction):
    ...
    def _calculate_junctions_mst_weight_using_air_distance(self, junctions: List[Junction]) -> float:
        ...
        """
        graph = nx.Graph()
        calc_distance_func = self.cached_air_distance_calculator.get_air_distance_between_junctions
        for junction_source in junctions:
            for junction_dest in junctions:
                if junction_source == junction_dest:
                    continue
                graph.add_edge(junction_source.index, junction_dest.index,
                               weight=calc_distance_func(junction_source, junction_dest))
...
class MDATestsTravelDistToNearestLabHeuristic(HeuristicFunction):
    ...
    def estimate(self, state: GraphProblemState) -> float:
        ...
        remain_apartments_waiting_to_visit = self.problem.get_reported_apartments_waiting_to_visit(state)
        test_travel_sum = 0
        if state.current_location not in self.problem.problem_input.laboratories:
            test_travel_sum += air_dist_to_closest_lab(state.current_location) *\
                               state.get_total_nr_tests_taken_and_stored_on_ambulance()

        for apartment in remain_apartments_waiting_to_visit:
            test_travel_sum += air_dist_to_closest_lab(apartment.location)*apartment.nr_roommates
        return test_travel_sum
