...
class MDAState(GraphProblemState):
    ...
    def __eq__(self, other):
        ...
        isinstance(other, MDAState)
        if self.current_location == other.current_location and \
...
class MDAProblem(GraphProblem):
    ...
    def expand_state_with_costs(self, state_to_expand: GraphProblemState) -> Iterator[OperatorResult]:
        ...
        for item in self.problem_input.laboratories:
            if item not in state_to_expand.visited_labs or len(state_to_expand.tests_on_ambulance) > 0:
                    indicator_matoshim = 0
                    if item not in state_to_expand.visited_labs:
                        indicator_matoshim = 1
                    new_mda_state = MDAState(item, frozenset(),
                                             ...
                                             item.max_nr_matoshim * indicator_matoshim,
                                             frozenset(state_to_expand.visited_labs | {item}))
                    new_operator_result = OperatorResult(new_mda_state,
                                                         self.get_operator_cost(state_to_expand, new_mda_state),
                                                         'go to lab ' + item.name)
                    yield new_operator_result

        for item in self.get_reported_apartments_waiting_to_visit(state_to_expand):
            if item is None:
                continue
            if (item not in state_to_expand.tests_on_ambulance | state_to_expand.tests_transferred_to_lab) and\
                    (item.nr_roommates <= state_to_expand.nr_matoshim_on_ambulance) and \
                    (item.nr_roommates <= self.problem_input.ambulance.total_fridges_capacity -
                     state_to_expand.get_total_nr_tests_taken_and_stored_on_ambulance()):
                new_mda_state = MDAState(item, frozenset(state_to_expand.tests_on_ambulance.union({item})),
                                         state_to_expand.tests_transferred_to_lab,
                                         state_to_expand.nr_matoshim_on_ambulance - item.nr_roommates,
                                         state_to_expand.visited_labs)
                new_operator_result = OperatorResult(new_mda_state,
                                                     self.get_operator_cost(state_to_expand, new_mda_state),
                                                     'visit ' + item.reporter_name)
                yield new_operator_result
    ...
    def get_operator_cost(self, prev_state: MDAState, succ_state: MDAState) -> MDACost:
        ...
        cost_distance = self.map_distance_finder.get_map_cost_between(prev_state.current_location,
        ...
        drive_per_meter = self.problem_input.ambulance.drive_gas_consumption_liter_per_meter
        gas_liter_price = self.problem_input.gas_liter_price
        fridge_capacity = self.problem_input.ambulance.fridge_capacity
        active_fridges = math.ceil(total_nr_tests/fridge_capacity)
        fridges_gas_per_meter = sum(self.problem_input.ambulance.fridges_gas_consumption_liter_per_meter[i]
                                    for i in range(active_fridges))
        cost_monetary = gas_liter_price * (drive_per_meter + fridges_gas_per_meter) * cost_distance
        if isinstance(succ_state.current_site, Laboratory):
            indicator_taken, indicator_is_visited = 0, 0
            if prev_state.tests_on_ambulance != frozenset():
                indicator_taken = 1
            if succ_state.current_site in prev_state.visited_labs:
                indicator_is_visited = 1
            cost_monetary += (indicator_taken * succ_state.current_site.tests_transfer_cost +
                              indicator_is_visited * succ_state.current_site.revisit_extra_cost)

        cost_test_travel = cost_distance * (sum(item.nr_roommates for item in prev_state.tests_on_ambulance))
        return MDACost(cost_distance, cost_monetary, cost_test_travel, self.optimization_objective)
    ...
    def is_goal(self, state: GraphProblemState) -> bool:
        ...
        union_apartments = state.tests_on_ambulance | state.tests_transferred_to_lab
        return state.current_site in self.problem_input.laboratories and \
            frozenset(self.problem_input.reported_apartments) == union_apartments
    ...
    # @staticmethod
    # def my_union(fs1, fs2):
    #     union_apartments = frozenset()
    #     if fs1 is None:
    #         if fs2 is not None:
    #             return fs2
    #     if fs2 is None:
    ...
    def get_reported_apartments_waiting_to_visit(self, state: MDAState) -> List[ApartmentWithSymptomsReport]:
        ...
        """
        #union_apartment = MDAProblem.my_union(state.tests_on_ambulance, state.tests_transferred_to_lab)
        union_apartment = frozenset(state.tests_on_ambulance | state.tests_transferred_to_lab)
        my_list = list(frozenset(self.problem_input.reported_apartments) - union_apartment)
        return sorted(my_list, key=lambda x: x.report_id)
    ...
    def get_all_certain_junctions_in_remaining_ambulance_path(self, state: MDAState) -> List[Junction]:
        ...
        my_list = [i.location for i in waiting_to_visit]
        my_list += [state.current_location]
